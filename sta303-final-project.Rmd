...
- Are there unusual discrepancies in the quality of the data measured by MINGAR fitness wearables for customers of different races that get differing amounts of sleep?
...
Below is a plot (blah blah blah).


As we are primarily interested in finding measuring the quality of the data, we have chosen to set number of flags as our predicted value. Naturally, as we are trying to understand how the race of a customer and their amount of sleep can affect the quality of the data measured by the device, we would set `emoji_modifier` and `duration` as our fixed random variables. After performing several likelihood ratio tests, we decided to also add `sex` and `age` as random variables.
...
```{r, echo=FALSE, fig.cap="Box-plots that show the distribution of the sleep quality for different number of flags and different emojis used.", fig.height=8}
