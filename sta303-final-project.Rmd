...
inc <- cust_dev_info %>%
  ...
  xlab("Median Income (in Canadian dollars)") +
    labs(caption = "Boxplots displaying incomes of customers who purchased \nMINGAR's new affordable line vs. \ncustomers who purchased MINGAR's other lines \n(i.e. 'Traditional Customers').") +
  theme_minimal() +
...
age <- cust_dev_info %>%
  ...
  xlab("Age (in years)") +
  labs(caption = "Boxplots displaying ages of customers who purchased \nMINGAR's new affordable line (i.e. 'New Customers') vs. \ncustomers who purchased MINGAR's other lines \n(i.e. 'Traditional Customers').") +
  theme_minimal() +
...
sleep <- cust_sleep_complete %>%
  ...
  labs(caption = "Scatterplots showing the relationship between duration of sleep session and number of flags for \neach emoji modifier.") +
  theme_minimal() +
  theme(plot.title = element_text(size = 12, hjust = 0.5),
        plot.caption = element_text(face = "italic", hjust = 0)) 
```

```{r, warning=FALSE, message=FALSE, echo=FALSE, fig.height=8.5}
